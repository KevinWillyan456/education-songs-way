    ...
    try {
        ...
        playlistsBefore.forEach(async (field) => {
            ...
            if (field.gender == 'Favorite') {
                ...
                if (user != null) {
                    totalVideos = user?.favoriteVideos.length
                } else {
                    totalVideos = 0
                }
            } else {
                totalVideos = await Video.count({ gender: field.gender })
            }
...
async function deletePlaylistAndVideos(
    req: Request<{ id?: UpdateWithAggregationPipeline }>,
    res: Response
) {
    const { id } = req.params
    ...
    try {
        const playlist = await Playlist.findById(id)
        const videos = await Video.find({ gender: playlist?.gender })
        const videoIdsToRemove = videos.map((video) => video._id)
        await Video.deleteMany({ gender: playlist?.gender })
        await User.updateMany(
            {},
            {
                $pull: {
                    favoriteVideos: { videoId: { $in: videoIdsToRemove } },
                    videoHistory: { videoId: { $in: videoIdsToRemove } },
                },
            }
        )
        await User.updateMany(
        ...
        return res
            .status(200)
            .json({ message: 'Playlist and videos removed successfully!' })
    } catch (err) {
        return res.status(500).json({ error: err })
    }
}
...
async function selectPlaylist(req: Request, res: Response) {
    ...
    try {
        if (playlist == 'Favorite') {
            const user: IUser | null = await User.findById(id, '-password')
            const userFavoriteVideos: string[] = []

            if (user != null) {
                user.favoriteVideos.forEach((video) => {
                    userFavoriteVideos.push(video.videoId)
                })
            }

            const videos = await Video.find({ _id: { $in: userFavoriteVideos } })
                .sort({ title: 1 })
                .collation({ locale: 'pt', strength: 2 })
        ...
        const videos = await Video.find({ gender: playlist })
            .sort({ title: 1 })
            .collation({ locale: 'pt', strength: 2 })
